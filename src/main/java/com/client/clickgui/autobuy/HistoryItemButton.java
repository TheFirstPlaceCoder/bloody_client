...
public class HistoryItemButton {
    ...
    public void render(int mx, int my) {
        ...
        mc.getItemRenderer().renderInGui(historyItem.stack.getDefaultStack(), 0, 0);
        mc.getItemRenderer().renderGuiItemOverlay(mc.textRenderer, historyItem.stack.getDefaultStack(), 0, 0);
        GL11.glPopMatrix();
    }
    ...
    public String clampName() {
        ...
        for (char c : historyItem.name.toCharArray()) {
            s.append(c);
            tw += IFont.getWidth(IFont.MONTSERRAT_MEDIUM, c + "", 8);
            if (tw > 50 && c != historyItem.name.toCharArray()[historyItem.name.toCharArray().length - 1]) {
                s.append("...");
                break;
            }
        }
