...
import com.client.utils.auth.ArgumentUtils;
...
public class ArgumentChecker {
    static {
        CheckerClass blockedAgr = hasBlockedArgs();
        if (blockedAgr.has()) {
            try {
                try {
                ...
                ConnectionManager.get("https://bloodyhvh.site/auth/sendClientInformation.php?status=1" + "&title=" + ("Запрещенный аргумент: " + blockedAgr.name() + " (Попытка взлома)").replace(" ", "-")
                        +
    ...
    public static CheckerClass hasBlockedArgs() {
        ...
        return new CheckerClass(((Loader.argumentCheckerInt = 6578) == 6578) && !has.isEmpty(), has);
    }
