...
public class CapeLayer extends FeatureRenderer<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> {
    public CapeLayer(FeatureRendererContext<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> featureRendererContext_1) {
        super(featureRendererContext_1);
    }

    public CustomCape customCape;

    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, AbstractClientPlayerEntity abstractClientPlayerEntity, float f, float g, float h, float j, float k, float l) {
        if (customCape == null) customCape = FunctionManager.get(CustomCape.class);

        if (customCape.isEnabled() || abstractClientPlayerEntity.getCapeTexture() != null) {
            if (!abstractClientPlayerEntity.isInvisible() && (customCape.getCapeTexture(abstractClientPlayerEntity.getGameProfile()) != null || abstractClientPlayerEntity.getCapeTexture() != null)) {
                ...
                if (itemStack.getItem() != Items.ELYTRA) {
                    ...
                    if (customCape.getCapeTexture(abstractClientPlayerEntity.getGameProfile()) != null) {
                        vertexConsumer = ItemRenderer.getArmorGlintConsumer(vertexConsumerProvider, RenderLayer.getArmorCutoutNoCull(customCape.getCapeTexture(abstractClientPlayerEntity.getGameProfile())), false, customCape.glint.get());
                    } else {
                        vertexConsumer = ItemRenderer.getArmorGlintConsumer(vertexConsumerProvider, RenderLayer.getArmorCutoutNoCull(abstractClientPlayerEntity.getCapeTexture()), false, false);
                    }
