package com.client.utils.render.gl;
...
public class Shader {
    ...
    public Shader(String vertexPath, String fragmentPath) {
        ...
        if (!compiled) {
            ...
            BloodyClient.LOGGER.error("Failed to compile vertex shader ({}):\n{}", vertexPath, error);
            return;
        }
        ...
        if (!compiled) {
            ...
            BloodyClient.LOGGER.error("Failed to compile fragment shader ({}):\n{}", fragmentPath, error);
            return;
        }
    ...
    public void set(String name, Color color) {
        set(name, color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha() / 255f);
    }

    private String read(String path) {
        try {
            InputStream in = mc.getResourceManager().getResource(new Identifier("bloody-client", path)).getInputStream();
