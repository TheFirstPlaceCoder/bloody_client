...
public class FontRenderer implements Closeable {
    ...
    private static boolean color, shouldRename = true;

    public static void color(boolean set) {
        color = set;
    }

    public static void shouldRename(boolean set) {
        shouldRename = set;
    }
    ...
    public static Identifier randomIdentifier() {
        return new Identifier("temp", randomString());
    }
    ...
    public void drawString(MatrixStack stack, String s, float x, float y, float r, float g, float b, float a, boolean shadow) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (glyph.value() != ' ' && glyph.value() != '█') {
                ...
                if (color && !hasStyle) {
                    Color c2 = Colors.getColor(Colors.getIndex(chars.length - i, chars.length * 4));
                    r2 = c2.getRed() / 255f;
                    g2 = c2.getGreen() / 255f;
                    b2 = c2.getBlue() / 255f;
                }
