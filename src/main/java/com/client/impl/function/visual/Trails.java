...
public class Trails extends Function {
    ...
    private final ListSetting mode = List().name("Режим").enName("Mode").defaultValue("Новый").list(List.of("Старый", "Новый")).build();
    public final DoubleSetting time = Double().name("Время жизни").enName("Life Time").defaultValue(3.4).min(0).max(10.0).build();
    public final DoubleSetting size = Double().name("Размер").enName("Size").defaultValue(3.0).min(0).max(5).visible(() -> mode.get().equals("Новый")).build();
