...
public class SwingAnimation extends Function {
    public final ListSetting swingAnimation = List().list(List.of("Свайп", "Строгий", "Ванильный")).name("Режим").enName("Mode").defaultValue("Свайп").build();
    public final BooleanSetting onlyAura = Boolean().name("Только с Attack Aura").enName("Only Attack Aura").defaultValue(false).build();
    public final IntegerSetting swingPower = Integer().name("Сила взмаха").enName("Swing Power").defaultValue(12).min(2).max(30).build();
    public final DoubleSetting scale = Double().name("Размер").enName("Size").defaultValue(1d).min(0.1).max(2).build();
