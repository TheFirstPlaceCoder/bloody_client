    ...
    private void resell() {
        ...
        if (mc.currentScreen instanceof GenericContainerScreen containerScreen) {
            for (int i = 0; i < containerScreen.getScreenHandler().getInventory().size(); i++) {
                String s = mode.get().equals("FunTime") ? "Хранилище" : "Товары на продаже";
                if (containerScreen.getScreenHandler().getInventory().getStack(i).getName().getString().contains(s)) {
            ...
            if (self) {
                ...
                for (int i = 0; i < containerScreen.getScreenHandler().getInventory().size(); i++) {
                    ...
                    if (stack.isEmpty() || name.equals("◀ Вернуться в главное меню")
                            || name.equals("Следующая страница ▶") || name.equals("◀ Назад") || name.contains("Помощь") || name.equals("◀ Предыдущая страница")
