...
public class Nuker extends Function {
    private final ListSetting mode = List().name("Режим").enName("Mode").list(List.of("Обычный", "Быстрый")).defaultValue("Обычный").build();
    public final DoubleSetting radius_hor = Double().name("Радиус по горизонтали").enName("Horizontal Radius").defaultValue(4.5).min(0).max(7).build();
    public final DoubleSetting radius_ver = Double().name("Радиус по вертикали").enName("Vertical Radius").defaultValue(2.0).min(0).max(7).build();
    public final DoubleSetting radius_mine = Double().name("Радиус ломания").enName("Break Range").defaultValue(4.5).min(0).max(6).build();
    public final IntegerSetting delay = Integer().name("Задержка ломания").enName("Break Delay").defaultValue(10).min(0).max(200).visible(() -> mode.get().equals("Быстрый")).build();
    ...
    public void onEnable() {
        ...
        nukerThread.setRunnable(this::runnable);
