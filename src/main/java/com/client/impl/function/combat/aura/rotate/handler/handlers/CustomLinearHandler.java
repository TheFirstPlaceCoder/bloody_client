...
public class CustomLinearHandler extends Handler {
    public CustomLinearHandler() {
        super("CustomLinear");
    ...
    public void tick(Entity target, double range) {
        ...
        rotate.a = (float) Utils.lerp(rotate.a, vec.a, Utils.random(aura.speedYaw.get() - aura.randCoef.get(), aura.speedYaw.get() + aura.randCoef.get()));

        if (!aura.onlyNotLook.get() || ((IGameRenderer) mc.gameRenderer).getTarget(rotate.a, rotate.b) != target)
            rotate.b = (float) Utils.lerp(rotate.b, vec.b, Utils.random(aura.speedPitch.get() - aura.randCoef.get(), aura.speedPitch.get() + aura.randCoef.get()));
    }
