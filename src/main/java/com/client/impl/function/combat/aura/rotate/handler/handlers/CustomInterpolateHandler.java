...
public class CustomInterpolateHandler extends Handler {
    public CustomInterpolateHandler() {
        super("CustomInterpolate");
    ...
    public void tick(Entity target, double range) {
        ...
        if (target != null) {
            ...
            incrementTicks += 0.05;
            rotate.a = interpolates.calculateCustomInterpolate(aura.interpolate.get(), rotate.a, targetRotation.a, incrementTicks);

            if (((IGameRenderer) mc.gameRenderer).getTarget(rotate.a, rotate.b) != target)
                rotate.b = (float) Utils.lerp(rotate.b, targetRotation.b, aura.pitchSpeed.get() / 10);
        }
    }
    ...
    public V2F getBestPoint(Entity target, double range) {
        ...
        Vec3d vec = new Vec3d(target.getX(), target.getY() + target.getHeight() * (target.getY() > mc.player.getY() ? 0.3F : 0.8F), target.getZ());
