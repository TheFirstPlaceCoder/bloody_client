...
public class ItemCooldown extends Function {
    ...
    public final ListSetting mode = List().name("Режим").list(List.of("ReallyWorld", "Задержка")).defaultValue("ReallyWorld").build();
    public final IntegerSetting delay = Integer().name("Задержка").defaultValue(4).min(0).max(10).visible(() -> !mode.get().equals("ReallyWorld")).build();
